  ...
  const getActivityBgColor = (activityType: ActivityType) => {
    ...
    const renderActivityRow = (activity: ActivityItem) => (
      <div
        ...
        className="px-4 py-1.5 hover:bg-gray-50 transition-colors"
      >
      <div className="flex items-center justify-between gap-4">
        <div className="flex items-center gap-2.5 flex-1 min-w-0">
          <div className={`p-1 ${getActivityBgColor(activity.activityType)} rounded-lg shrink-0 flex items-center justify-center`}>
              ...
              <div className="flex flex-col flex-1 overflow-hidden">
                <div className="bg-white rounded-lg shadow-sm border border-gray-200 flex-1 flex flex-col overflow-hidden">
                  <div className="flex-1 overflow-y-auto divide-y divide-gray-200">
                    {paginatedPaymentPlanActivities.map(renderActivityRow)}
                  </div>
                </div>
                {totalPaymentPlanActivities > 0 && (
                  <div className="flex items-center justify-between mt-4 bg-white rounded-lg shadow-sm border border-gray-200 px-4 py-3 shrink-0">
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        onClick={() => setCurrentPagePaymentPlans(prev => Math.max(1, prev - 1))}
                        disabled={totalPagesPaymentPlans === 1 || currentPagePaymentPlans === 1}
                        ...
                        <div className="flex items-center gap-1">
                          {Array.from({ length: totalPagesPaymentPlans }, (_, i) => i + 1).map((page) => {
                            if (
                              ...
                              (page >= currentPagePaymentPlans - 1 && page <= currentPagePaymentPlans + 1)
                            ) {
                              return (
                                <button
                                  key={page}
                                  onClick={() => setCurrentPagePaymentPlans(page)}
                                  className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors cursor-pointer ${
                                    currentPagePaymentPlans === page
                      ...
                      <button
                        onClick={() => setCurrentPagePaymentPlans(prev => Math.min(totalPagesPaymentPlans, prev + 1))}
                        disabled={totalPagesPaymentPlans === 1 || currentPagePaymentPlans === totalPagesPaymentPlans}
                        className="p-2 rounded-lg border border-gray-300 hover:bg-gray-50 transition-colors cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:bg-transparent"
          ...
          <div className="flex flex-col overflow-hidden">
            ...
            {unitDesignActivities.length === 0 ? (
              ...
              <div className="flex flex-col flex-1 overflow-hidden">
                <div className="bg-white rounded-lg shadow-sm border border-gray-200 flex-1 flex flex-col overflow-hidden">
                  <div className="flex-1 overflow-y-auto divide-y divide-gray-200">
                    {paginatedUnitDesignActivities.map(renderActivityRow)}
                  </div>
                </div>
                {totalUnitDesignActivities > 0 && (
                  <div className="flex items-center justify-between mt-4 bg-white rounded-lg shadow-sm border border-gray-200 px-4 py-3 shrink-0">
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        onClick={() => setCurrentPageUnitDesigns(prev => Math.max(1, prev - 1))}
                        disabled={totalPagesUnitDesigns === 1 || currentPageUnitDesigns === 1}
                        className="p-2 rounded-lg border border-gray-300 hover:bg-gray-50 transition-colors cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:bg-transparent"
                        aria-label="Previous page"
                      >
                        <ChevronLeft className="w-4 h-4" />
                        ...
                        <div className="flex items-center gap-1">
                          {Array.from({ length: totalPagesUnitDesigns }, (_, i) => i + 1).map((page) => {
                            if (
                              ...
                              (page >= currentPageUnitDesigns - 1 && page <= currentPageUnitDesigns + 1)
                            ) {
                              return (
                                <button
                                  key={page}
                                  onClick={() => setCurrentPageUnitDesigns(page)}
                                  className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors cursor-pointer ${
                                    currentPageUnitDesigns === page
                                      ? "bg-blue-600 text-white"
                                      : "bg-gray-100 text-gray-700 hover:bg-gray-200"
                                  }`}
                                >
                      ...
                      <button
                        onClick={() => setCurrentPageUnitDesigns(prev => Math.min(totalPagesUnitDesigns, prev + 1))}
                        disabled={totalPagesUnitDesigns === 1 || currentPageUnitDesigns === totalPagesUnitDesigns}
                        className="p-2 rounded-lg border border-gray-300 hover:bg-gray-50 transition-colors cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:bg-transparent"
                        aria-label="Next page"
                      >
                        <ChevronRight className="w-4 h-4" />
