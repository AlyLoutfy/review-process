...
export default function UnitDesignListView({ 
  unitDesign, 
  isReviewed, 
  onToggleReviewed, 
  hasIssues, 
  ...
  return (
    <>
      <div className={`rounded-2xl shadow-sm hover:shadow-xl transition-all duration-300 p-6 relative overflow-hidden ${
        isReviewed 
          ? "bg-gradient-to-br from-green-50 to-green-100/50 border-2 border-green-400/60" 
          : hasIssues
          ? "bg-gradient-to-br from-red-50 to-red-100/50 border-2 border-red-400/60"
          : "bg-white border border-gray-200 hover:border-blue-300 hover:shadow-lg hover:bg-gradient-to-br hover:from-blue-50/30 hover:to-white"
      }`}>
        {/* Subtle background accent */}
        {!isReviewed && !hasIssues && (
          <div className="absolute top-0 right-0 w-32 h-32 bg-gradient-to-br from-blue-100/20 to-transparent rounded-full blur-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
        )}
        ...
        <div className="absolute top-5 right-5 flex items-center gap-2 z-10">
          <div className="relative z-50">
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-2 rounded-lg transition-all duration-200 cursor-pointer relative group ${
                hasIssues
                  ? "bg-red-100 hover:bg-red-200 border border-red-300 shadow-sm"
                  : "bg-white hover:bg-red-50 border border-gray-300 hover:border-red-300 shadow-sm"
              }`}
            ...
            >
              <Flag 
                className={`w-4 h-4 transition-colors ${
                  hasIssues ? "text-red-700 fill-red-700" : "text-gray-600 group-hover:text-red-600"
                }`}
              />
              <span className={`text-xs font-semibold ${
                hasIssues ? "text-red-700" : "text-gray-600 group-hover:text-red-600"
              }`}>
                {hasIssues ? "View Issue" : "Flag Issue"}
              </span>
            </button>
            {hasIssues && latestIssue && (
              <div 
                className={`fixed w-72 p-3 bg-gray-900 text-white text-sm rounded-lg shadow-2xl transition-all duration-200 z-[9999] ${
                  isTooltipVisible ? "opacity-100 visible scale-100 pointer-events-auto" : "opacity-0 invisible scale-95 pointer-events-none"
                }`}
                style={{
              ...
              >
                ...
                {latestIssue.fileName && (
                  <div className="mt-2 text-xs text-gray-400">File: {latestIssue.fileName}</div>
                )}
          ...
          <button
            ...
            className={`group flex items-center gap-1.5 px-3 py-2 rounded-lg transition-all duration-200 cursor-pointer ${
              isReviewed
                ? "bg-green-100 hover:bg-green-200 border border-green-300 shadow-sm"
                : "bg-white hover:bg-green-50 border border-gray-300 hover:border-green-300 shadow-sm"
            }`}
          ...
          >
              <CheckCircle2 
                className={`w-4 h-4 transition-colors ${
                  isReviewed ? "text-green-700 fill-green-700" : "text-gray-600 group-hover:text-green-600"
                }`}
              />
              <span className={`text-xs font-semibold ${
                isReviewed ? "text-green-700" : "text-gray-600 group-hover:text-green-600"
              }`}>
        ...
        <div className="flex items-start gap-8 relative">
          ...
          <div className="flex-shrink-0 w-64 pr-8 border-r border-gray-200/60">
            <h3 className={`text-xl font-bold mb-4 tracking-tight ${
              isReviewed ? "text-gray-400 line-through" : "text-gray-900"
            }`}>
              {unitDesign.name}
            </h3>
            ...
            {unitDesign.amenities && unitDesign.amenities.length > 0 && (
              <div className="flex flex-wrap gap-1 mb-3">
                {unitDesign.amenities.map((amenity, idx) => (
                  <span
                    key={idx}
                    className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-medium bg-blue-50 text-blue-700 border border-blue-200/60"
                  >
                    {amenity}
                  </span>
                ))}
            ...
            <div className="space-y-2.5 mb-4">
              <div className="flex items-center gap-2.5">
                ...
                <span className={`text-sm font-bold ${
                  isReviewed ? "text-gray-400" : "text-gray-900"
                }`}>
                  {unitDesign.beds === 0 ? "Studio" : `${unitDesign.beds} Bed${unitDesign.beds > 1 ? "s" : ""}`}
                </span>
              </div>
              <div className="flex items-center gap-2.5">
              ...
              <div className="flex items-center gap-2.5 pt-3 border-t border-gray-200/60">
                ...
                <span className={`text-sm font-bold ${
                  isReviewed ? "text-gray-400" : "text-blue-600"
                }`}>
                  {unitCount}
                </span>
                {unitCount > 0 && (
            ...
            <div className="flex flex-col gap-2">
              <button
                ...
                className={`flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg transition-all duration-200 text-xs font-medium cursor-pointer h-9 ${
                  isReviewed
                    ? "bg-gray-100 text-gray-500 hover:bg-gray-200 border border-gray-200"
                    : "bg-gradient-to-r from-blue-50 to-blue-100/50 text-blue-700 hover:from-blue-100 hover:to-blue-200 border border-blue-200/60 shadow-sm hover:shadow-md"
                }`}
              >
      ...
      <IssueModal
        ...
        itemName={unitDesign.name}
        initialText={latestIssue?.text || ""}
        initialFileName={latestIssue?.fileName || null}
      />

      <ConfirmResolveIssueModal
        isOpen={isConfirmModalOpen}
        onClose={() => setIsConfirmModalOpen(false)}
        onConfirm={handleConfirmResolve}
        issueText={latestIssue?.text || ""}
        itemName={unitDesign.name}
      />
      ...
      <UnitIDsModal
        isOpen={isUnitIDsModalOpen}
        onClose={() => setIsUnitIDsModalOpen(false)}
        unitDesignName={unitDesign.name}
        unitIds={unitIds}
      />
    </>
  );
