...
export default function PaymentPlanCard({ plan, isReviewed, onToggleReviewed, hasIssues, latestIssue, onIssueSubmit, onResolveIssue, onDeleteIssue, units, unitDesigns }: PaymentPlanCardProps) {
  ...
  return (
    <>
      <div className={`rounded-lg shadow-sm hover:shadow-md transition-all duration-300 p-4 border-2 ${
        isReviewed 
          ? "bg-green-50/60 border-green-400/60" 
          : hasIssues
          ? "bg-red-50/60 border-red-400/60"
          : "bg-white border-transparent hover:border-blue-400/60 hover:bg-gray-50/50"
      }`}>
        <div className="flex flex-col h-full">
          <div className="flex items-start justify-between mb-3">
            <h3 className={`text-lg font-semibold flex-1 break-words ${
              isReviewed ? "text-gray-700 line-through" : "text-gray-900"
            }`}>
              {plan.name}
            </h3>
            <div className="flex flex-col items-end gap-2 ml-2 flex-shrink-0">
              <button
                ...
                className={`group flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg transition-all duration-200 cursor-pointer w-full justify-center ${
                  isReviewed
                    ? "bg-green-100 hover:bg-green-200 border border-green-300"
                    : "bg-white hover:bg-green-50 border border-gray-300 hover:border-green-300"
                }`}
              ...
              <div className="relative z-50">
                <button
                  ...
                  className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg transition-all duration-200 cursor-pointer relative group w-full justify-center ${
                    hasIssues
                      ? "bg-red-100 hover:bg-red-200 border border-red-300"
                      : "bg-white hover:bg-red-50 border border-gray-300 hover:border-red-300"
                  }`}
        ...
        <div className="flex-1 mb-3 space-y-2">
          <div className="flex items-baseline gap-2">
            ...
            <span className={`text-base font-bold ${
              isReviewed ? "text-gray-500" : "text-blue-600"
            }`}>
              {plan.downPayment}%
            </span>
          </div>
          <div className="flex items-center gap-2">
            ...
            <span className={`text-sm font-semibold ${
              isReviewed ? "text-gray-500" : "text-gray-900"
            }`}>
          ...
          <div className="flex flex-wrap items-center gap-3 pt-2 border-t border-gray-200">
            <div className="flex items-center gap-2">
              ...
              <span className={`text-xs ${
                plan.includesMaintenance ? "text-green-700 font-medium" : "text-gray-500"
              }`}>
                Maintenance
              </span>
            </div>
            <div className="flex items-center gap-2">
              ...
              <span className={`text-xs ${
                plan.includesClubFees ? "text-green-700 font-medium" : "text-gray-500"
              }`}>
                Club Fees
              </span>
            </div>
        ...
        <button
          ...
          className={`flex items-center justify-center gap-2 w-full px-3 py-2 rounded-lg transition-all duration-200 text-sm font-medium cursor-pointer ${
            isReviewed
    ...
    <UnitSelectionModal
      isOpen={isUnitSelectionModalOpen}
      onClose={() => setIsUnitSelectionModalOpen(false)}
      onSelectUnit={handleUnitSelect}
      units={units}
      unitDesigns={unitDesigns}
    />
    </>
  );
}
